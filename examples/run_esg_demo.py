"""Run the synthetic ESG demo without requiring an API key."""
...
def main() -> None:
    job_path = PROJECT_ROOT / "domains" / "esg" / "job.json"
    output_path = PROJECT_ROOT / "outputs" / "esg_demo_result.json"
    result = run_workflow(job_path)
    write_json(output_path, result)

    evaluation = result.get("evaluation_summary", {})
    print("Dictionary-driven ESG demo completed")
